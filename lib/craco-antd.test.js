...
test("the webpack config is modified correctly when loading vars from Less file", () => {
  ...
  expect(mockReadLess.mock.calls[0][0]).toBe("./antd.customize.less");
...
test("the webpack config is modified correctly when loading vars from JSON file", () => {
  ...
  expect(mockReadJSON.mock.calls[0][0]).toBe("./antd.customize.json");
